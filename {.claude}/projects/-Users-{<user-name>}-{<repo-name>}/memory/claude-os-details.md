...
memory/*.md + ~/.claude/ configs
  -> 6-gc.sh (every 7d)  -> scores files (0-100), archives <30, flags 30-59 for consolidation
...
|--------|-------------|-----------|
| **checkpoint.sh** | Filters live files (strips URLs, replaces project-specific values with placeholders), merges into ~/claude-os/ templates using line-level union. Commits locally. | CMM -> claude-os |
| **bootstrap.sh** | Pulls templates from ~/claude-os/ into a new project. Replaces placeholders. Generates launchd plists. Runs install.sh. Also syncs slash commands, scripts, and git hooks. | claude-os -> CMM |
...
| `git` | checkpoint, bootstrap | For template sync |
| `launchctl` | bootstrap | Generates + loads launchd plists |
...
| `~/claude-os/{<repo-name>}/` | bootstrap | checkpoint |
